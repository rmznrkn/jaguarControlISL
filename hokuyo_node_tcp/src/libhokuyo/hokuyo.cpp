...
hokuyo::Laser::laserReadline(char *buf, int len, int timeout)
{
  ...
  while(true)
  {
    bytes = tcpclient_readline(&tcpclient,buf, len, timeout);
    if (bytes == -1 && errno != EAGAIN && errno != EWOULDBLOCK)
       HOKUYO_EXCEPT(hokuyo::Exception,"read failed errno: %d",errno);
    if(bytes > 0)
    {
      return bytes;
    }
    if(bytes < 0 && (errno == EAGAIN || errno == EWOULDBLOCK))
      ROS_WARN("Errno: %d", errno);
  }
  return bytes;
#else
...
long long int hokuyo::Laser::getHokuyoScanStampToSystemStampOffset(bool intensity, double min_ang, double max_ang, int clustering, int skip, int reps, int timeout)
{
  ...
  for (int i = 0; i < reps; i++)
  {
    //printf("serviceScan %d start\n", i);
    serviceScan(scan, timeout);
    //printf("serviceScan %d end\n", i);
...
hokuyo::Laser::calcLatency(bool intensity, double min_ang, double max_ang, int clustering, int skip, int num, int timeout)
{
  ...
  for (int i = 0; i < num; i++)
  {
    //printf("getHokuyoScanStampToSystemStampOffset stared\n");
    ...
    //printf("getHokuyoScanStampToSystemStampOffset end\n");
    //printf("getHokuyoClockOffset stared\n");
